...
def generate_music_sync(request: MusicGenerationRequest):
    ...
    try:
        ...
        try:
            # 장르와 가사를 임시 파일로 저장
            genre_file = tempfile.NamedTemporaryFile(mode='w+', suffix='.txt', delete=False)
            genre_file.write(request.genre_txt)
            genre_file.close()
            
            lyrics_file = tempfile.NamedTemporaryFile(mode='w+', suffix='.txt', delete=False)
            ...
            # 생성된 MP3 파일 경로
            output_file_path = os.path.join(DEFAULT_OUTPUT_DIR, DEFAULT_OUTPUT_FILENAME)
            
            # 파일이 존재하는지 확인
            if not os.path.exists(output_file_path):
                raise HTTPException(status_code=500, detail="생성된 음악 파일을 찾을 수 없습니다.")
